...
public class ExceptionAware<T> {
    ...
    protected T handleValidationException(ValidationException e) {
        ...
        } else if (e.getInvalidValues().length == 1) {
    ...
    private String createMessage(InvalidValue invalidValue) {
        if (hasMessage(invalidValue.getMessage())) {
            ...
            return getMessage(invalidValue.getMessage(), new Object[] { invalidValue.getBeanClass(),
                invalidValue.getPropertyPath(), invalidValue.getValue() });
        } else {
    ...
    protected String getMessage(String errorCode, Object[] parameters) {
        try {
            ...
            return getMessagesAccessor().getMessage(transformedErrorCode, parameters, "??? " + transformedErrorCode + " ???");
        } catch (Exception e) {
            return null;
        }
    }
