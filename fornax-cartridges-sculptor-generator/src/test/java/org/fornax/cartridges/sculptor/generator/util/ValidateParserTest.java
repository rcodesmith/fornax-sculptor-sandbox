...
public class ValidateParserTest {
    ...
    public void customLength() {
        ...
        assertEquals("@org.hibernate.validator.Length(min=2, max=9)", result);
    }
    ...
    public void several() {
        attribute.setNullable(true);
        attribute.setValidate("@Length(min=2, max=9) @EAN @notEmpty");
        String result = GenerationHelper.getValidationAnnotations(attribute);
        assertEquals(
                "@org.hibernate.validator.Length(min=2, max=9) " +
