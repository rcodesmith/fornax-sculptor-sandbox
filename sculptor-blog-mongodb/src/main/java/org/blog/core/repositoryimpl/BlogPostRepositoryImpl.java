...
import static org.blog.core.domain.BlogPostProperties.comments;
...
public class BlogPostRepositoryImpl extends BlogPostRepositoryBase {
    ...
    public List<BlogPost> findPostsWithGreatComments() {
        List<ConditionalCriteria> condition = criteriaFor(BlogPost.class).withProperty(comments().title())
    ...
    public List<BlogPost> findPostsInBlog(Blog blog) {
        List<ConditionalCriteria> condition = criteriaFor(BlogPost.class).withProperty(inBlog()).eq(blog).orderBy(
                published()).descending().build();
        return findByCondition(condition);
    }
}
